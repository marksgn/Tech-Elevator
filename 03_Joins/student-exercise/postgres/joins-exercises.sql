...
INNER JOIN film_category fc ON f.film_id = fc.film_id
INNER JOIN category ON fc.category_id = category.category_id
WHERE category.name = 'Comedy' AND f.rating = 'G' ;
...
-- (2 rows)
SELECT f.title, category.name, a.first_name, a.last_name
FROM film f
INNER JOIN film_category fc ON f.film_id = fc.film_id
INNER JOIN category ON fc.category_id = category.category_id
INNER JOIN film_actor fa ON fa.film_id = f.film_id
...
SELECT c.first_name, c.last_name, COUNT(C.CUSTOMER_ID)as numofrental
FROM customer c
JOIN payment p ON p.customer_id = c.customer_id 
GROUP BY c.first_name, c.last_name
ORDER BY numofrental DESC LIMIT 10;
