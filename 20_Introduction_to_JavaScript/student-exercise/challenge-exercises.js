...
*/
    ...
    function iqTest(numbers){
        //seperate numbers and make readable
        numbers = numbers.split(" ").map(function(x){return parseInt(x)});
        
        //determine odd numbers in list
        let odd = numbers.filter(function(x){ return x % 2 === 1});
        console.log(odd);

        //determine even numbers in list
        let even = numbers.filter(function(x){ return x % 2 === 0});
        console.log(even);
...
*/
    ...
    function titleCase(title, minorWords) {
        let arrTitle = title.split(' ').map(word => word.toLowerCase()).map(word => word[0].toUpperCase() + word.substring(1));
        
        if (minorWords) {
            let arrMinorWords = minorWords.toLowerCase().split(' ');
            arrTitle = arrTitle.map((word, index) => {
                if (arrMinorWords) {
                    if (index === 0) {
                    return word;
                    } 
                    if (arrMinorWords === 0) {
                        return word;
                    } else if (arrMinorWords.includes(word.toLowerCase())) {
