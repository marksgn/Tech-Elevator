		...
		try (Scanner fileReader = new Scanner(itemInventory)) {
			while (fileReader.hasNextLine()) {
				...
				if(!line.isEmpty()) {
					...
					//the file has the locations for each item in the machine writtern on the first part of each line
					// set each letter of that first line to the designated item category and add it to an arraylist of all the Items
