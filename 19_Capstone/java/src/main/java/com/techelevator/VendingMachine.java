...
public class VendingMachine {
	...
	// display inventory taken from file reader list
	...
	// accept users money and determine balance by adding inserted money value to balance
	// write each entry into the logger
	...
	public void Purchase(String userChoice) throws FileNotFoundException {
		// determine if selected item even exists or if it's out of stock because they bought all of them already
		if (!inventory.containsKey(userChoice)) {
			...
			//make sure the price is in the same format at the inserted money	
			//subtract purchase cost from overall balance
			//if the inserted money is less than the price, ask for more money
