...
public class Exercises {
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		int peterOriginal = peterPaul.get("Peter");
		int paulOriginal = peterPaul.get("Paul");
		
		if (peterOriginal >= 5000 && paulOriginal >= 10000) {
			
			int partnershipAmount = 0;
			int peterQuarter = peterOriginal / 4;
			int paulQuarter = paulOriginal / 4;

			peterOriginal = peterOriginal - peterQuarter; //add peter's contribution to PeterPaulPartnership
			peterPaul.put("Peter", peterOriginal); //remove peter's contribution from peter
			
			paulOriginal = paulOriginal - paulQuarter;
			peterPaul.put("Paul", paulOriginal);
			
			partnershipAmount = peterQuarter + paulQuarter; 
			
			peterPaul.put("PeterPaulPartnership", partnershipAmount);
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for (String strings : words) {
				key.put(strings.substring(0,1), strings.substring(strings.length()-1));
		}
		return key;
	}
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		...
		for (int intValue : ints) {
			if (counts.containsKey(intValue)) {
				int newCount = counts.get(intValue);
				newCount++;
				counts.put(intValue, newCount);
			}
			else {
				counts.put(intValue, 1);
			}
		}
		return counts;
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		for (String multipleTimes : words) {
			if(twoOrMore.containsKey(multipleTimes)) {
				int newCount = twoOrMore.get(multipleTimes);
				newCount++;
					if (newCount > 1) {
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse, Map<String, Integer> remoteWarehouse) {
		...
		for (String i : mainWarehouse.keySet()) {
			if(remoteWarehouse.containsKey(i)) {
				newMap.put(i, (mainWarehouse.get(i) + remoteWarehouse.get(i)));
			} else {
				newMap.put(i, mainWarehouse.get(i));
			}
		}
		for (String i : remoteWarehouse.keySet()) {
			if (!(mainWarehouse.containsKey(i))) {
				newMap.put(i,  remoteWarehouse.get(i));
			}
		}
		return newMap;
	}
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		...
		Map<String, Integer> keyCount = new HashMap<String, Integer>();
			...
			//cycle thru the String[] 
				//each iteration, i need to figure out what last two letters are and assign to a variable
			//compare created variable to each character in the string
			//translate to our math we created
			//need a count variable too
	...
	public List<String> distinctValues(List<String> stringList) {
		...
		Set<String> set = new LinkedHashSet<String>(stringList);
		
		/*alternatively loop through the list and add to a set
		Set<String> set2 = new LinkedHashSet(stringList);
		...
		*/
		...
		/*alternatively loop through the lsit and add to a set
