...
public class LectureTest {
	...
	//this will make ANY test fail if it's longer than 5000 ms
	@Rule
	...
	public void startsWith_returns_true_if_a_string_starts_with_the_specified_characters() {
		...
		boolean startsWithHello = theString.startsWith("ello"); // Act
	...
	@Test
	public void compare_doubles_with_delta() {
		Double one = 1.0;
		Double aLittleBigger = 1.00000001;
		
		//doubles are imprecise so the assertequals lets us put in a delta
		Assert.assertEquals(one, aLittleBigger,.001);
	}
	...
	@Test(timeout=11)
	public void test_finishes_execution_in_a_timely_manner() {
		long startTime = System.currentTimeMillis();

		//doReallyLongThing();
		//pause for 11 milliseconds
		try {
			Thread.sleep(11);
		...
		Assert.assertTrue("This took too long! < 10 ms",(endTime - startTime) < 10);
