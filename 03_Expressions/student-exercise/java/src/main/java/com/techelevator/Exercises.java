...
public class Exercises {
	...
	public boolean sleepIn(boolean weekday, boolean vacation) {
		//if weekday && vacation = sleep in (true)
		//!weekday = sleep in (true)
		if ((vacation == true) || (!weekday)) {
			return true;
		}

		return false;
	}
		// can also write  --  return((vacation == true) || (weekday == false))
	...
	public boolean monkeyTrouble(boolean aSmile, boolean bSmile) {
		if ((aSmile == true && bSmile == true) || (!aSmile && !bSmile)) {
			return true;
		}
				return false;
	}
	...
	public int caughtSpeeding(int speed, boolean isBirthday) {
		int birthdaySpecial = isBirthday ? 5 : 0;
		if (speed <= 60 + birthdaySpecial) {
			return 0;
		}
		else if (speed > 60 + birthdaySpecial && speed <= 80 + birthdaySpecial) {
			return 1;
		}
		else {
			return 2;
	...
	public boolean specialEleven(int n) {
		 ...
		 * return isMult || isOneMoreThanMult
		*/
	...
	public int luckySum(int a, int b, int c) {
		int sum = 0;
		//a counts if a is not 13
		if (a!=13) {
			sum += a;
		}
		//b counts is b is not 13 and a is not 13
		if (b!=13 && a!=13) {
			sum += b;
		}
		//c counts if c is not 13 and b is not 13
		if (c!=13 && b!=13) {
			sum += c;
		}
		return sum;
