...
document.addEventListener('DOMContentLoaded', () => {
  ...
  //listen for click on description
  const desc = document.querySelector('.description')
  desc.addEventListener('click',(event) => {
    toggleDescriptionEdit(event.target)
  })

  //listen on teh inputDesc textbox for if the user hits enter or escape
  ...
  //add an event listener so if the mouse leaves the text box
  //field it's the same as hitting escape
  inputDesc.addEventListener('mouseleave',(event)=>{
    exitDescriptionEdit(event.target,false)
  })

  //grab the add review button and put an event lstener on it
  const btnToggleForm = document.getElementById('btnToggleForm')
  btnToggleForm.addEventListener('click',showHideForm)

  //make save the form button work
...
function saveReview() {
  ...
  //make a new review out of all those details
  const newReview = {
    reviewer: name.value,
    title: title.value,
    review: review.value,
