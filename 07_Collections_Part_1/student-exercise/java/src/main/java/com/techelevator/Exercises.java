...
public class Exercises {
	...
	public List<String> reverseList(List<String> stringList) {
		...
		for (String popColors: stringList) {
	...
	public Integer findLargest(List<Integer> integerList) {
		...
		//found this simpler version to solve this too - return Collections.max(integerList);
	}
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		...
		for (Integer modeTheOdds: integerArray) {
			if (modeTheOdds % 2 != 0) {
					onlyOdd.add(modeTheOdds);
			}
		}
		return onlyOdd;
	}
	...
	** INTERVIEW QUESTION **
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		// create my list of strings myStringList
		List<String> fizzBuzzList = new ArrayList<String>();
		
		// loop through integerArray each element i
		for (Integer i : integerArray) {
			if (i%3 == 0 && i%5 == 0) {
				//add "FizzBuzz" to list of strings
				fizzBuzzList.add("FizzBuzz");
			}
			else if (i%3 == 0) {
				//add "Fizz" to the list of strings
				fizzBuzzList.add("Fizz");
			}
			else if (i%5 == 0) {
				//add "Buzz" to the list of strings
				fizzBuzzList.add("Buzz");
			}
			else {
				//add the i to the list of strings
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		//loop through my seatNumberList, for each seatNumber
			//figure out which queue it belongs in
			//and offer it to that queue
		for (Integer i : seatNumberList) {
			...
			if (i >= 21 && i <= 30) {
				twentyOneThru30.add(i);
			}
		}
		
		List<Integer> combo = new ArrayList<Integer>();
		while(oneThru10.size() > 0) {
			combo.add(oneThru10.poll());
		}
		while(elevenThru20.size() > 0) {
			combo.add(elevenThru20.poll());
		}
		while(twentyOneThru30.size() > 0) {
			combo.add(twentyOneThru30.poll());
		}
		return combo;
	}
